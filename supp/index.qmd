...
The elliptical bifaces were run a second time, then coded as ColorGroup A and ColorGroup B. Images of the bifaces are not included here due to the fact that they originated from Caddo burial contexts. To test whether the bifaces differ in color, the color distance matrix was exported, joined with the qualitative data, then analyzed using a permutational multivariate analysis of variance (perMANOVA) in the `vegan` package, where the color groups were found to differ significantly (permutations = 10,000; Rsq = 0.60308; Pr(>F) = 0.0011). A non-metric multidimensional scaling plot (NMDS) was used to illustrate the color ranges for elliptical bifaces from ColorGroup A and ColorGroup B. The 12 whole/complete elliptical bifaces were subsequently used in an analysis of biface morphology between raw material color groups.
